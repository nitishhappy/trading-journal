...
export default async function handler(req, res) {
  ...
  try {
    // Primary: Binance BTCUSDT 5m klines
    let candles = [];
    ...
    // Fallback: Yahoo Finance BTC-USD 5m
    if (candles.length === 0) {
    ...
    if (candles.length === 0) {
      return res.status(200).json({ success: false, candles: [], message: "No BTC candle data available." });
    }
